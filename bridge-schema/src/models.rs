...
use starcoin_bridge_indexer_builder::{Task, LIVE_TASK_TARGET_BLOCK};
use std::str::FromStr;
use strum_macros::{AsRefStr, EnumString};

use crate::schema::{
    governance_actions, progress_store, starcoin_bridge_error_transactions,
    starcoin_bridge_progress_store, token_transfer, token_transfer_data,
};
...
impl From<ProgressStore> for Task {
    fn from(value: ProgressStore) -> Self {
        Self {
            ...
            start_block: value.block_number as u64,
            target_block: value.target_block as u64,
            // Ok to unwrap, timestamp is defaulted to now() in database
            timestamp: value.timestamp.expect("Timestamp not set").0 as u64,
            is_live_task: value.target_block == LIVE_TASK_TARGET_BLOCK,
        }
    }
}
