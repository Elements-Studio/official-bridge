...
import "../contracts/StarcoinBridge.sol";
...
contract DeployBridge is Script {
    ...
    function parseDeployConfig(string memory path) public returns (DeployConfig memory) {
        ...
        config.starcoinDecimals = abi.decode(vm.parseJson(json, ".starcoinDecimals"), (uint256[]));
        config.weth = abi.decode(vm.parseJson(json, ".weth"), (address));
        // claimDelaySeconds: default to 10 seconds if not specified
        if (vm.keyExistsJson(json, ".claimDelaySeconds")) {
            config.claimDelaySeconds = abi.decode(vm.parseJson(json, ".claimDelaySeconds"), (uint256));
        } else {
            config.claimDelaySeconds = 10;
        }
    ...
    function run() external {
        ...
        if (isLocal) {
            ...
            deployConfig.starcoinDecimals = new uint256[](5);
            deployConfig.starcoinDecimals[0] = 9;
            deployConfig.starcoinDecimals[1] = 8;
            deployConfig.starcoinDecimals[2] = 8;
            deployConfig.starcoinDecimals[3] = 6;
            deployConfig.starcoinDecimals[4] = 6;

            deployConfig.tokenPrices = new uint256[](5);
            deployConfig.tokenPrices[0] = 0; // STC
            deployConfig.tokenPrices[1] = 0; // BTC
            deployConfig.tokenPrices[2] = 0; // ETH
            deployConfig.tokenPrices[3] = 0; // USDC
            deployConfig.tokenPrices[4] = 100000000; // USDT
        }
        ...
        require(
            deployConfig.supportedTokens.length == deployConfig.starcoinDecimals.length,
            "supportedTokens.length != starcoinDecimals.length"
        );
        ...
        address bridgeConfig = _deployBridgeConfig(bridgeCommittee, deployConfig, supportedChainIds);
        ...
        // deploy Starcoin Bridge ========================================================================

        address starcoinBridge = Upgrades.deployUUPSProxy(
            "StarcoinBridge.sol",
            abi.encodeCall(StarcoinBridge.initialize, (bridgeCommittee, address(vault), limiter)),
            opts
        );

        // transfer vault ownership to bridge
        vault.transferOwnership(starcoinBridge);
        // transfer limiter ownership to bridge
        BridgeLimiter instance = BridgeLimiter(limiter);
        instance.transferOwnership(starcoinBridge);

        // print deployed addresses for post deployment setup
        console.log("[Deployed] BridgeConfig:", bridgeConfig);
        console.log("[Deployed] StarcoinBridge:", starcoinBridge);
    ...
    function _deployBridgeConfig(
        ...
        // convert Starcoin Decimals from uint256 to uint8
        uint8[] memory starcoinDecimals = new uint8[](deployConfig.starcoinDecimals.length);
        for (uint256 i; i < deployConfig.starcoinDecimals.length; i++) {
            starcoinDecimals[i] = uint8(deployConfig.starcoinDecimals[i]);
        }
        ...
        return Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                ...
                (
                    ...
                    starcoinDecimals,
                    supportedChainIds,
                    uint64(deployConfig.claimDelaySeconds)
                )
            ),
            opts
        );
