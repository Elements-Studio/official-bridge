...
contract BridgeConfigTest is BridgeBaseTest {
    ...
    function testBridgeConfigInitialization() public {
        ...
        assertEq(config.tokenStarcoinDecimalOf(0), 9);
        assertEq(config.tokenStarcoinDecimalOf(1), 8);
        assertEq(config.tokenStarcoinDecimalOf(2), 8);
        assertEq(config.tokenStarcoinDecimalOf(3), 6);
        assertEq(config.tokenStarcoinDecimalOf(4), 6);
        assertEq(config.tokenPriceOf(0), STC_PRICE);
    ...
    function testTokenStarcoinDecimalOf() public {
        assertEq(config.tokenStarcoinDecimalOf(1), 8);
    }
    ...
    function testAddTokensWithSignatures() public {
        ...
        uint8 _numStarcoinDecimals = 1;
        uint8 starcoinDecimal1 = 6;
        uint8 _numPrices = 1;
        ...
        assertEq(config.tokenStarcoinDecimalOf(10), 6);
    ...
    function testAddTokensStarcoinDecimalFailure() public {
        MockUSDC _newToken = new MockUSDC();
        ...
        // add token shoudl fail because the starcoin decimal is greater than the eth decimal
        vm.expectRevert(bytes("BridgeConfig: Invalid Starcoin decimal"));
        config.addTokensWithSignatures(signatures, message);
    }
    ...
    function testUpdateTokenPricesRegressionTest() public {
        ...
        bytes memory expectedEncodedMessage =
            hex"53544152434f494e5f4252494447455f4d4553534147450401000000000000010a0301000000003b9aca00";
    ...
    function testUpdateTokenPriceRegressionTestWithSigVerficiation() public {
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                ...
                (address(committee), chainID, supportedTokens, tokenPrices, tokenIds, starcoinDecimals, _supportedDestinationChains, 10)
            ),
            opts
        );
        ...
        bytes memory encodedMessage = BridgeUtils.encodeMessage(message);
        bytes memory expectedEncodedMessage =
            hex"53544152434f494e5f4252494447455f4d455353414745040100000000000000000b010000000023c34600";
    ...
    function testAddTokensRegressionTest() public {
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                BridgeConfig.initialize,
                (address(committee), 12, supportedTokens, tokenPrices, tokenIds, starcoinDecimals, _supportedDestinationChains, 10)
            ),
            opts
        );
        config = BridgeConfig(_config);
        ...
        assertEq(starcoinDecimals.length, 3);
        assertEq(starcoinDecimals[0], 5);
        assertEq(starcoinDecimals[1], 6);
        assertEq(starcoinDecimals[2], 7);
        ...
        bytes memory encodedMessage = BridgeUtils.encodeMessage(message);
        bytes memory expectedEncodedMessage =
            hex"";
        ...
        assertEq(config.tokenStarcoinDecimalOf(99), 5);
        assertEq(config.tokenStarcoinDecimalOf(100), 6);
        assertEq(config.tokenStarcoinDecimalOf(101), 7);
