...
// Helper contract to wrap library internal functions for testing reverts
contract BridgeUtilsWrapper {
    function convertERC20ToStarcoinDecimal(uint8 erc20Decimal, uint8 starcoinDecimal, uint256 amount) external pure returns (uint64) {
        return BridgeUtils.convertERC20ToStarcoinDecimal(erc20Decimal, starcoinDecimal, amount);
    }
    
    function convertStarcoinToERC20Decimal(uint8 erc20Decimal, uint8 starcoinDecimal, uint64 amount) external pure returns (uint256) {
        return BridgeUtils.convertStarcoinToERC20Decimal(erc20Decimal, starcoinDecimal, amount);
    }
    
    function requiredStake(BridgeUtils.Message memory _message) external pure returns (uint32) {
        return BridgeUtils.requiredStake(_message);
    }
}

contract BridgeUtilsTest is BridgeBaseTest {
    BridgeUtilsWrapper wrapper;
    
    // This function is called before each unit test
    function setUp() public {
        setUpBridgeTest();
        wrapper = new BridgeUtilsWrapper();
    }

    function testConvertERC20ToStarcoinDecimalAmountTooLargeForUint64() public {
        vm.expectRevert(bytes("BridgeUtils: Amount too large for uint64"));
        wrapper.convertERC20ToStarcoinDecimal(18, 8, type(uint256).max);
    }

    function testConvertERC20ToStarcoinDecimalInvalidStarcoinDecimal() public {
        vm.expectRevert(bytes("BridgeUtils: Invalid Starcoin decimal"));
        wrapper.convertERC20ToStarcoinDecimal(10, 11, 100);
    }

    function testconvertStarcoinToERC20DecimalInvalidStarcoinDecimal() public {
        vm.expectRevert(bytes("BridgeUtils: Invalid Starcoin decimal"));
        wrapper.convertStarcoinToERC20Decimal(10, 11, 100);
    }

    function testConvertERC20ToStarcoinDecimal() public {
        // ETH
        assertEq(IERC20Metadata(wETH).decimals(), 18);
        uint256 ethAmount = 10 ether;
        uint64 starcoinAmount = BridgeUtils.convertERC20ToStarcoinDecimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.ETH)).decimals(),
            config.tokenStarcoinDecimalOf(BridgeUtils.ETH),
            ethAmount
        );
        assertEq(starcoinAmount, 10_000_000_00); // 10 * 10 ^ 8

        // USDC
        assertEq(IERC20Metadata(USDC).decimals(), 6);
        ethAmount = 50_000_000; // 50 USDC
        starcoinAmount = BridgeUtils.convertERC20ToStarcoinDecimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.USDC)).decimals(),
            config.tokenStarcoinDecimalOf(BridgeUtils.USDC),
            ethAmount
        );
        assertEq(starcoinAmount, ethAmount);
        ...
        starcoinAmount = BridgeUtils.convertERC20ToStarcoinDecimal(
            ...
            config.tokenStarcoinDecimalOf(BridgeUtils.USDT),
            ethAmount
        );
        assertEq(starcoinAmount, ethAmount);
        ...
        starcoinAmount = BridgeUtils.convertERC20ToStarcoinDecimal(
            ...
            config.tokenStarcoinDecimalOf(BridgeUtils.BTC),
            ethAmount
        );
        assertEq(starcoinAmount, ethAmount);
    }

    function testconvertStarcoinToERC20Decimal() public {
        // ETH
        assertEq(IERC20Metadata(wETH).decimals(), 18);
        uint64 starcoinAmount = 11_000_000_00; // 11 eth
        uint256 ethAmount = BridgeUtils.convertStarcoinToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.ETH)).decimals(),
            config.tokenStarcoinDecimalOf(BridgeUtils.ETH),
            starcoinAmount
        );
        ...
        // USDC
        assertEq(IERC20Metadata(USDC).decimals(), 6);
        starcoinAmount = 50_000_000; // 50 USDC
        ethAmount = BridgeUtils.convertStarcoinToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.USDC)).decimals(),
            config.tokenStarcoinDecimalOf(BridgeUtils.USDC),
            starcoinAmount
        );
        ...
        // USDT
        assertEq(IERC20Metadata(USDT).decimals(), 6);
        starcoinAmount = 50_000_000; // 50 USDT
        ethAmount = BridgeUtils.convertStarcoinToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.USDT)).decimals(),
            config.tokenStarcoinDecimalOf(BridgeUtils.USDT),
            starcoinAmount
        ...
        // BTC
        assertEq(IERC20Metadata(wBTC).decimals(), 8);
        starcoinAmount = 3_000_000_00; // 3 BTC
        ethAmount = BridgeUtils.convertStarcoinToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(BridgeUtils.BTC)).decimals(),
            config.tokenStarcoinDecimalOf(BridgeUtils.BTC),
            starcoinAmount
    ...
    function testEncodeMessage() public {
        bytes memory moveEncodedMessage = abi.encodePacked(
            hex"53544152434f494e5f4252494447455f4d45535341474500010000000000000000011080ab1ee086210a3a37355300ca24672e0b14b18f79fe671db47393315ffdb377da4ea1b7af96010084d71700000000"
        );

        uint64 nonce = 0;
        uint8 starcoinChainId = 1;

        bytes memory payload = abi.encodePacked(
            hex"1080ab1ee086210a3a37355300ca24672e0b14b18f79fe671db47393315ffdb377da4ea1b7af96010084d71700000000"
        );
    ...
    function testDecodeTransferTokenPayload() public {
        // 10: sender length 1 bytes (16 = 0x10)
        // 80ab1ee086210a3a37355300ca24672e: sender 16 bytes (Starcoin address)
        ...
        bytes memory payload =
            hex"1080ab1ee086210a3a37355300ca24672e0b14b18f79fe671db47393315ffdb377da4ea1b7af9602000000c70432b1dd";

        BridgeUtils.TokenTransferPayload memory _payload =
            BridgeUtils.decodeTokenTransferPayload(payload);

        assertEq(_payload.senderAddressLength, uint8(16));
        assertEq(
            _payload.senderAddress,
            hex"80ab1ee086210a3a37355300ca24672e"
        );
    ...
    function testDecodeUpgradePayloadWith1ArgRegressionTest() public {
        bytes memory newMockFunc1Message =
            hex"";
    ...
    function testDecodeUpgradePayloadWith2ArgsRegressionTest() public {
        bytes memory newMockFunc2Message =
            hex"";
    ...
    function testDecodeUpgradePayloadWithNoCalldataRegressionTest() public {
        bytes memory emptyCalldataMessage =
            hex"";
    ...
    function testrequiredStakeInvalidType() public {
        ...
        wrapper.requiredStake(BridgeUtils.Message(invalidType, 0, 0, 1, bytes(hex"00")));
    }
}
