...
/// @notice This contract manages the committee members of the StarcoinBridge. The committee members are
...
contract BridgeCommittee is IBridgeCommittee, CommitteeUpgradeable {
    ...
    /// @notice Emitted when a new committee member is added.
    /// @param nonce The nonce of the add member message.
    /// @param member The address of the new member.
    /// @param stakeAmount The stake amount of the new member.
    event MemberAdded(uint64 nonce, address indexed member, uint16 stakeAmount);
    ...
    /// @notice Adds a new committee member if provided signatures are valid.
    /// @param signatures The array of signatures to validate the message.
    /// @param message BridgeUtils containing the add member payload.
    function addMemberWithSignatures(
        bytes[] memory signatures,
        BridgeUtils.Message memory message
    )
        external
        nonReentrant
        verifyMessageAndSignatures(message, signatures, BridgeUtils.ADD_MEMBER)
    {
        // decode the add member payload
        (address newMember, uint16 stakeAmount) =
            BridgeUtils.decodeAddMemberPayload(message.payload);

        // check if member already exists
        require(committeeStake[newMember] == 0, "BridgeCommittee: Member already exists");

        // add the new member
        committeeStake[newMember] = stakeAmount;
        committeeIndex[newMember] = uint8(message.nonce); // use nonce as index for simplicity

        emit MemberAdded(message.nonce, newMember, stakeAmount);
    }
