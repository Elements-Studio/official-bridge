...
interface IBridgeConfig {
    ...
    function tokenStarcoinDecimalOf(uint8 tokenID) external view returns (uint8);
    ...
    /// @notice Returns the claim delay in seconds.
    /// @dev This is the minimum time that must pass between approval and claim.
    function claimDelaySeconds() external view returns (uint64);
    ...
    /// @param starcoinDecimals The added token's decimal places on Starcoin.
    /// @param tokenPrices The prices of the tokens added in USD.
    event TokensAddedV2(
        uint64 nonce,
        uint8[] tokenIDs,
        address[] tokenAddresses,
        uint8[] starcoinDecimals,
        uint64[] tokenPrices
    );

    /// @dev (deprecated in favor of TokensAddedV2)
    event TokenAdded(uint8 tokenID, address tokenAddress, uint8 starcoinDecimal, uint64 tokenPrice);
