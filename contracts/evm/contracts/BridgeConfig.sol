...
/// @title BridgeConfig
/// @notice This contract manages a registry of supported tokens and supported chain IDs for the StarcoinBridge.
/// It also provides functions to convert token amounts to Starcoin decimal adjusted amounts and vice versa.
contract BridgeConfig is IBridgeConfig, CommitteeUpgradeable {
    ...
    // Claim delay in seconds (time between approval and claim)
    ...
    /// @param _claimDelaySeconds The delay in seconds between approval and claim (immutable after init).
    function initialize(
        address _committee,
        uint8 _chainID,
        ...
        uint8[] memory _starcoinDecimals,
        uint8[] memory _supportedChains,
        uint64 _claimDelaySeconds
        ...
        require(
            _supportedTokens.length == _starcoinDecimals.length, "BridgeConfig: Invalid Starcoin decimals"
        );
        // Max delay: 30 days to prevent permanent fund lock
        require(_claimDelaySeconds > 0, "BridgeConfig: Claim delay must be greater than zero");
        require(_claimDelaySeconds <= 30 days, "BridgeConfig: Claim delay exceeds maximum");
        ...
        for (uint8 i; i < _tokenIds.length; i++) {
            ...
            supportedTokens[_tokenIds[i]] = Token(_supportedTokens[i], _starcoinDecimals[i], true);
        }
        ...
        chainID = _chainID;
        claimDelaySeconds = _claimDelaySeconds;
    }
    ...
    /// @notice Returns the starcoin decimal places of the token with the given ID.
    /// @param tokenID The ID of the token.
    /// @return amount of starcoin decimal places of the provided token.
    function tokenStarcoinDecimalOf(uint8 tokenID) public view override returns (uint8) {
        return supportedTokens[tokenID].starcoinDecimal;
    }
    ...
    /// @notice Returns whether a token is supported in StarcoinBridge with the given ID.
    /// @param tokenID The ID of the token.
    ...
    /// @notice Returns whether a chain is supported in StarcoinBridge with the given ID.
        ...
        verifyMessageAndSignatures(message, signatures, BridgeUtils.ADD_EVM_TOKENS)
    {
        ...
        for (uint8 i; i < tokenIDs.length; i++) {
            _addToken(tokenIDs[i], tokenAddresses[i], starcoinDecimals[i], _tokenPrices[i], native);
        }

        emit TokensAddedV2(message.nonce, tokenIDs, tokenAddresses, starcoinDecimals, _tokenPrices);
    }
    ...
    /// @param starcoinDecimal The decimal places of the token.
    /// @param tokenPrice The price of the token.
    /// @param native Whether the token is native to the chain.
    function _addToken(
        uint8 tokenID,
        address tokenAddress,
        uint8 starcoinDecimal,
        uint64 tokenPrice,
        bool native
    ) private {
        require(tokenAddress != address(0), "BridgeConfig: Invalid token address");
        require(starcoinDecimal > 0, "BridgeConfig: Invalid Starcoin decimal");
        require(tokenPrice > 0, "BridgeConfig: Invalid token price");

        uint8 erc20Decimals = IERC20Metadata(tokenAddress).decimals();
        require(erc20Decimals >= starcoinDecimal, "BridgeConfig: Invalid Starcoin decimal");

        supportedTokens[tokenID] = Token(tokenAddress, starcoinDecimal, native);
        tokenPrices[tokenID] = tokenPrice;
    }
