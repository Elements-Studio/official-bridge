...
/// @title BridgeUtils
/// @notice This library defines the message format and constants for the Starcoin native bridge. It also
...
library BridgeUtils {
    ...
    uint8 public constant ADD_MEMBER = 8;

    // Message type stake requirements
    uint32 public constant TRANSFER_STAKE_REQUIRED = 2;
    ...
    uint32 public constant ADD_MEMBER_STAKE_REQUIRED = 5001;

    // token Ids
    uint8 public constant STC = 0;
    ...
    function requiredStake(Message memory _message) internal pure returns (uint32) {
        ...
        } else if (_message.messageType == ADD_MEMBER) {
            return ADD_MEMBER_STAKE_REQUIRED;
        } else {
            revert("BridgeUtils: Invalid message type");
        }
    }

    /// @notice Converts the provided token amount to the Starcoin decimal adjusted amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param starcoinDecimal The starcoin decimal value for the token.
    /// @param amount The ERC20 amount of the tokens to convert to Starcoin.
    /// @return Starcoin converted amount.
    function convertERC20ToStarcoinDecimal(uint8 erc20Decimal, uint8 starcoinDecimal, uint256 amount)
        internal
        pure
        returns (uint64)
    {
        if (erc20Decimal == starcoinDecimal) {
        ...
        require(erc20Decimal > starcoinDecimal, "BridgeUtils: Invalid Starcoin decimal");

        // Difference in decimal places
        uint256 factor = 10 ** (erc20Decimal - starcoinDecimal);
    ...
    /// @notice Converts the provided Starcoin decimal adjusted amount to the ERC20 token amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param starcoinDecimal The starcoin decimal value for the token.
    /// @param amount The Starcoin amount of the tokens to convert to ERC20.
    /// @return ERC20 converted amount.
    function convertStarcoinToERC20Decimal(uint8 erc20Decimal, uint8 starcoinDecimal, uint64 amount)
        internal
        pure
        returns (uint256)
    {
        if (starcoinDecimal == erc20Decimal) {
    ...
    /// @notice Decodes an add member payload from bytes.
    /// @dev The function will revert if the payload length is invalid.
    ///     Add member payload is 22 bytes.
    ///     bytes 0-19   : member address (20 bytes)
    ///     bytes 20-21  : stake amount (uint16, 2 bytes)
    /// @param _payload The payload to be decoded.
    /// @return member The address of the new member.
    /// @return stakeAmount The stake amount for the new member.
    function decodeAddMemberPayload(bytes memory _payload)
        internal
        pure
        returns (address member, uint16 stakeAmount)
    {
        require(_payload.length == 22, "BridgeUtils: Invalid payload length");
        
        // Extract address (20 bytes)
        assembly {
            member := mload(add(_payload, 20))
        }
        
        // Extract stake amount (uint16, bytes 20-21)
        stakeAmount = uint16(uint8(_payload[20])) << 8 | uint16(uint8(_payload[21]));
    }
    ...
    ///     byte m + 1       : number of starcoin decimals
    ///     bytes m + 2 -> i : starcoin decimals
    ...
    /// @return starcoinDecimals the Starcoin decimal places of the tokens to be added.
        ...
        )
    {
        ...
        starcoinDecimals = new uint8[](decimalCount);
        for (uint8 i; i < decimalCount; i++) {
            starcoinDecimals[i] = uint8(_payload[offset++]);
        }
