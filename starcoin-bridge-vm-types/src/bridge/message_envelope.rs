...
use super::committee::CommitteeVersion;
use super::crypto::{AuthoritySignInfo, AuthoritySignature, EmptySignInfo, Signer};
...
{
    pub fn new<S>(
        committee_version: CommitteeVersion,
        data: T,
        secret: &S,
    ...
    {
        let auth_signature = Self::sign(committee_version, &data, secret, authority);
        Self {
            digest: OnceCell::new(),
            data,
            auth_signature,
    ...
    pub fn committee_version(&self) -> CommitteeVersion {
        self.auth_signature.committee_version
    }
}
