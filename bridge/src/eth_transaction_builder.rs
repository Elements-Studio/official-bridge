...
use crate::types::{
    ...
    BridgeCommitteeValiditySignInfo, EvmAddMemberAction, EvmContractUpgradeAction,
...
pub async fn build_eth_transaction(
    ...
    if !action.is_governace_action() {
        return Err(BridgeError::InternalError(
            "Action is not a governance action".to_string(),
        ));
    }
    // TODO: Check chain id?
    ...
    match action.data() {
        ...
        BridgeAction::UpdateCommitteeMemberAction(_) => {
            // UpdateCommitteeMember is a Starcoin-only action
            unreachable!("UpdateCommitteeMemberAction is not supported on ETH");
        }
        BridgeAction::EvmAddMemberAction(action) => {
            build_add_member_approve_transaction(contract_address, signer, action.clone(), sigs)
                .await
        }
    }
}
...
pub async fn build_add_member_approve_transaction(
    contract_address: EthAddress,
    signer: EthSigner,
    action: EvmAddMemberAction,
    sigs: &BridgeCommitteeValiditySignInfo,
) -> BridgeResult<ContractCall<EthSigner, ()>> {
    let contract = EthBridgeCommittee::new(contract_address, signer.into());
    let message: eth_bridge_committee::Message = action.try_into()?;
    let signatures = sigs
        .signatures
        .values()
        .map(|sig| Bytes::from(sig.as_ref().to_vec()))
        .collect::<Vec<_>>();
    Ok(contract.add_member_with_signatures(signatures, message))
}
