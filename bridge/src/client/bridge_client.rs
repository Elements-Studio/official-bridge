...
impl BridgeClient {
    ...
    fn bridge_action_to_path(event: &BridgeAction) -> String {
        match event {
            ...
            BridgeAction::UpdateCommitteeMemberAction(a) => {
                let chain_id = (a.chain_id as u8).to_string();
                let nonce = a.nonce.to_string();
                let update_type = (a.update_type as u8).to_string();
                let bridge_pubkey = Hex::encode(&a.bridge_pubkey_bytes);
                let voting_power = a.voting_power.to_string();
                let http_rest_url = Hex::encode(a.http_rest_url.as_bytes());
                format!(
                    "sign/update_committee_member/{chain_id}/{nonce}/{update_type}/{bridge_pubkey}/{voting_power}/{http_rest_url}"
                )
            }
            BridgeAction::EvmAddMemberAction(a) => {
                let chain_id = (a.chain_id as u8).to_string();
                let nonce = a.nonce.to_string();
                let member_address = Hex::encode(a.member_address.as_bytes());
                let stake = a.stake.to_string();
                format!("sign/add_member_eth/{chain_id}/{nonce}/{member_address}/{stake}")
            }
        }
    }
    ...
    pub async fn request_sign_bridge_action(
        ...
        if !resp.status().is_success() {
            ...
            return match resp_text {
                // Match both old format (TxNotFinalized) and new format (TxNotFinalized(...))
                text if text.contains("TxNotFinalized") => {
                    // Create a generic TxNotFinalizedInfo since we can't parse the full details
                    // The server response contains the actual details
                    Err(BridgeError::TxNotFinalized(
                        crate::error::TxNotFinalizedInfo {
                            chain: "unknown".to_string(),
                            tx_block: 0,
                            finalized_block: 0,
                            blocks_to_finalize: 0,
