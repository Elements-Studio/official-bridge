...
impl CachedOpenFile {
    ...
    pub(crate) fn new<P: AsRef<Path>>(file_path: Option<P>) -> std::io::Result<Self> {
    ...
    pub(crate) fn update_path<P: AsRef<Path>>(&self, file_path: P) -> std::io::Result<()> {
...
impl FileExporter {
    pub(crate) fn new(file_path: Option<PathBuf>) -> std::io::Result<Self> {
        Ok(Self {
